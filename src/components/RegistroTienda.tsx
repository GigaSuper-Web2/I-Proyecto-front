...
const RegistroTienda = () => {
    ...
    const formlleno = async (e: React.FormEvent) => {
        ...
        if (!logo || !publicKeyFile || !signatureFile) {
            alert('Por favor, adjuntar el logo, archivo de firma digital y archivo de clave pública.');
            return;
        }
        ...
        try {
            ...
            const verifyResponse = await axios.post('http://localhost:5001/bank/validate-signature', verificationFormData);
            ...
            <div className="card p-4" style={{ maxWidth: '500px', width: '100%' }}>
                ...
                <form onSubmit={formlleno}>
                    ...
                    <div className="form-group">
                        ...
                        <input required type="text" placeholder="Nombre Empresa" className="form-control" value={nombre} onChange={(e) => setNombre(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Propietario empresa</label>
                        <input required type="text" placeholder="Propietario Empresa" className="form-control" value={propietarioEmpresa} onChange={(e) => setPropietarioEmpresa(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Cédula empresa</label>
                        <input required type="text" placeholder="Cédula Empresa" className="form-control" value={cedEmpresa} onChange={(e) => setCedEmpresa(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Categoría empresa</label>
                        <input required type="text" placeholder="Categoría" className="form-control" value={categoria} onChange={(e) => setCategoria(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Archivo de firma digital (.pem)</label>
                        <input required type="file" accept=".pem" className="form-control" onChange={handleSignatureFileChange} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Archivo de clave pública (.pem)</label>
                        <input required type="file" accept=".pem" className="form-control" onChange={(e) => setPublicKeyFile(e.target.files?.[0] || null)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Correo Electrónico</label>
                        <input required type="email" placeholder="Correo" className="form-control" value={correo} onChange={(e) => setCorreo(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Contraseña</label>
                        <input required type="password" placeholder="Contraseña" className="form-control" value={pass} onChange={(e) => setPassword(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Confirmar Contraseña</label>
                        <input required type="password" placeholder="Confirmar Contraseña" className="form-control" value={confirmPass} onChange={(e) => setConfirmPass(e.target.value)} style={{ marginBottom: '15px' }} />
                    </div>
                    <div className="form-group">
                        <label style={{ marginLeft: '5px' }}>Logo empresa</label>
                        <input type="file" accept=".svg" className="form-control" onChange={(e) => setLogo(e.target.files?.[0] || null)} />
                    </div>
